public class Ball {
    ...
    public void update(double dt){
        ...
        if (vy >= 0.0) {
            if (this.rect.y + (vy * dt) + this.rect.height > Constants.INSETS_BOTTOM){
                vy *= -1.0;
            }
        } else if (vy < 0.0){
        ...
        if (vx < 0.0){
            if (this.rect.x + (vx * dt) <= this.leftPaddle.x + this.leftPaddle.width){
              if(this.rect.y + (vy * dt) > this.leftPaddle.y &&
                      this.rect.y + (vy * dt) + this.leftPaddle.height < leftPaddle.y + leftPaddle.height){
